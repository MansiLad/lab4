...
const EventsList = () => {
  ...
  //const [searchTermPage, setSearchTermPage] = useState(`https://app.ticketmaster.com/discovery/v2/events?apikey=j4seKRXKtlDrFGapW4DMmcwIZvJqOFRH&keyword=${searchTerm}&countryCode=US`)
  const [searchData, setSearchData] = useState(undefined);
  let card = null;

  let [currentpage, setcurrentpage] = useState("https://app.ticketmaster.com/discovery/v2/events.json?countryCode=US&page=0&size=20&apikey=j4seKRXKtlDrFGapW4DMmcwIZvJqOFRH")
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const result = await axios.get(`https://app.ticketmaster.com/discovery/v2/events?apikey=j4seKRXKtlDrFGapW4DMmcwIZvJqOFRH&keyword=${searchTerm}&countryCode=US`);
 ...
 */
        ...
        <Card className={classes.card} variant='outlined'>
          <CardActionArea>
            ...
            <CardMedia
                ...
                component='img'
                image={
                  event.images && event.images[1].url
                    ? event.images[1].url
                    : noImage
                }
                title='event image'
              ...
              <CardContent>
                ...
                <Typography variant='body2' color='textSecondary' component='p'>
                  <p>Event Date: {
                          event && event.dates.start.dateTime ? (
                            <dd>{formatDate(event.dates.start.dateTime)}</dd>
                          ) : (
                            <dd>N/A</dd>
                          )
                        }
                    </p>
                    <p>Venue: {event._embedded.venues[0].name}, {event._embedded.venues[0].city.name}, {event._embedded.venues[0].state.stateCode}</p>
  ...
  return(
    ...
    <SearchEvents searchValue={searchValue} />
      ...
      {(page !== 1) && 
        <button>
          <Link to={`/events/page/${page-1}`} onClick={ gotoprevpage }> Previous </Link>
        </button>
      }
      <span> Page {page} </span>
      ...
      {gotonextpage && 
        <button>
          <Link to={`/events/page/${page+1}`} onClick={ gotonextpage }> Next </Link>
        </button>
      }
    </div>
    <br />
